...
const Profile: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetch = async () => {
            ...
            await axios.get(`http://localhost:5000/api/auth/profile/${res.id}`) // Adjust the endpoint as needed
                .then(response => {
    ...
    const handleCredentialSubmit = async (e: React.FormEvent) => {
        ...
        const url = credStatus ? `http://localhost:5000/api/credentials/updateCredentials/${res.id}` : `http://localhost:5000/api/credentials/addCredentials/${res.id}`;
